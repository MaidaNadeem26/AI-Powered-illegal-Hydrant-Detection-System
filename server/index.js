...
import { fetchProcessImage, searchCopernicus, validateSearchRequest } from './copernicus.js';
import { analyzeSatelliteImage } from './gemini.js';
import { toPotentialHotspot } from './hotspot.js';
import { addVerification, getHotspot, getStats, listHotspots, saveDetection, statuses, updateHotspot } from './db.js';
...
const writeLimit = new Map();
function requireAdmin(request, response, next) {
    const token = process.env.ADMIN_TOKEN;
    if (!token)
        return next();
    if (request.headers.authorization !== `Bearer ${token}`)
        return response.status(401).json({ error: 'Administrator authorization is required.' });
    next();
}
function rateLimit(request, response, next) {
    const key = request.ip ?? 'unknown';
    const current = writeLimit.get(key) ?? { count: 0, at: Date.now() };
    if (Date.now() - current.at > 60_000) {
        current.count = 0;
        current.at = Date.now();
    }
    current.count += 1;
    writeLimit.set(key, current);
    if (current.count > 20)
        return response.status(429).json({ error: 'Too many write requests. Try again shortly.' });
    next();
}
...
app.get('/api/hotspots', (request, response) => response.json(listHotspots(request.query)));
app.get('/api/hotspots/stats', (request, response) => response.json(getStats(request.query)));
app.get('/api/hotspots/export.csv', (request, response) => {
    const rows = listHotspots({ ...request.query, page: '1', pageSize: '10000' }).items;
    ...
    response.setHeader('Content-Disposition', 'attachment; filename="hotspots.csv"');
    response.send(csv);
});
app.get('/api/hotspots/:id', (request, response) => { const result = getHotspot(request.params.id); if (!result)
    return response.status(404).json({ error: 'Hotspot not found.' }); response.json(result); });
app.patch('/api/hotspots/:id', requireAdmin, rateLimit, (request, response) => { const body = z.object({ country: z.string().max(120).nullable(), region: z.string().max(120).nullable() }).safeParse(request.body); if (!body.success)
    return response.status(400).json({ error: 'Only country and region are editable.' }); const result = updateHotspot(request.params.id, body.data.country, body.data.region); if (!result)
    return response.status(404).json({ error: 'Hotspot not found.' }); response.json(result); });
...
app.post('/api/hotspots/:id/verifications', requireAdmin, rateLimit, upload.array('photos', 6), async (request, response) => {
    const parsed = verificationSchema.safeParse(request.body);
    if (!parsed.success)
        return response.status(400).json({ error: parsed.error.issues[0]?.message ?? 'Invalid verification fields.' });
    const files = request.files ?? [];
    const evidenceDirectory = path.join(path.dirname(fileURLToPath(new URL('.', import.meta.url))), 'uploads', 'evidence');
    ...
    for (const file of files) {
        ...
        if (!isJpeg && !isPng && !isWebp)
            return response.status(400).json({ error: 'Photos must be valid JPEG, PNG, or WebP images.' });
        const extension = isJpeg ? 'jpg' : isPng ? 'png' : 'webp';
        const filename = `${randomUUID()}.${extension}`;
        await writeFile(path.join(evidenceDirectory, filename), file.buffer);
        const metadata = isJpeg ? await exifr.parse(file.buffer, { tiff: true, exif: true, gps: true }) : null;
    ...
    try {
        const result = addVerification({ hotspotId: request.params.id, status: parsed.data.status, notes: parsed.data.notes, reviewerName: parsed.data.reviewer_name, visitedAt: parsed.data.visited_at, latitude: parsed.data.latitude ?? null, longitude: parsed.data.longitude ?? null, accuracy: parsed.data.location_accuracy_m ?? null, evidence });
        if (!result)
            return response.status(404).json({ error: 'Hotspot not found.' });
        response.status(201).json(result);
    }
    catch {
        response.status(404).json({ error: 'Hotspot not found.' });
    }
});
app.listen(port, () => {
