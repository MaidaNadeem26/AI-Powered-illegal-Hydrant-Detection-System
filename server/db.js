...
export function deriveStatus(hotspotId) {
    const row = db.prepare('SELECT status FROM verifications WHERE hotspot_id = ? ORDER BY created_at DESC LIMIT 1').get(hotspotId);
    return row?.status ?? 'pending';
}
function parseHotspot(row) {
...
export function saveDetection(input) {
    const timestamp = now();
    const nearby = db.prepare(`SELECT * FROM hotspots WHERE ((latitude - ?) * 111320) * ((latitude - ?) * 111320) + ((longitude - ?) * 111320 * cos(radians(latitude)) * (longitude - ?) * 111320 * cos(radians(latitude))) <= ?`).get(input.latitude, input.latitude, input.longitude, input.longitude, 300 * 300);
...
export function listHotspots(filters) {
    ...
    if (filters.minConfidence) {
        where.push('h.latest_confidence >= ?');
        values.push(Number(filters.minConfidence));
    }
    if (filters.q) {
        where.push('(h.country LIKE ? OR h.region LIKE ? OR h.latest_summary LIKE ?)');
...
export function getHotspot(id) {
    const row = db.prepare('SELECT * FROM hotspots WHERE id = ?').get(id);
    if (!row)
        return null;
    const detections = db.prepare('SELECT * FROM detections WHERE hotspot_id = ? ORDER BY created_at DESC').all(id);
    const verifications = db.prepare('SELECT * FROM verifications WHERE hotspot_id = ? ORDER BY created_at DESC').all(id);
    return { hotspot: parseHotspot(row), detections: detections.map((d) => ({ ...d, signs: JSON.parse(String(d.signs)) })), verifications: verifications.map((v) => ({ ...v, evidence: db.prepare('SELECT * FROM evidence WHERE verification_id = ?').all(v.id) })) };
}
export function updateHotspot(id, country, region) { db.prepare('UPDATE hotspots SET country = ?, region = ?, updated_at = ? WHERE id = ?').run(country, region, now(), id); return getHotspot(id); }
export function addVerification(input) {
    const id = randomUUID();
    const timestamp = now();
    const transaction = db.transaction(() => { db.prepare(`INSERT INTO verifications (id, hotspot_id, status, notes, reviewer_name, visited_at, latitude, longitude, location_accuracy_m, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.hotspotId, input.status, input.notes, input.reviewerName, input.visitedAt, input.latitude, input.longitude, input.accuracy, timestamp); const statement = db.prepare('INSERT INTO evidence (id, verification_id, file_path, mime_type, size_bytes, taken_at, original_name) VALUES (?, ?, ?, ?, ?, ?, ?)'); for (const item of input.evidence)
        statement.run(randomUUID(), id, item.filePath, item.mimeType, item.sizeBytes, item.takenAt, item.originalName); db.prepare('UPDATE hotspots SET status = ?, updated_at = ? WHERE id = ?').run(input.status, timestamp, input.hotspotId); });
    transaction();
