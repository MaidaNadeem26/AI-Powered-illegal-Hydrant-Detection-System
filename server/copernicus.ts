...
interface CopernicusProduct {
  ...
  Assets?: Array<{ Type?: string; DownloadLink?: string; DownloadUrl?: string }>
}
...
async function getCopernicusToken() {
  ...
  if (username && password) {
    ...
    if (!response.ok) throw new Error(`Copernicus user authentication returned HTTP ${response.status}. Check CDSE_USERNAME and CDSE_PASSWORD.`)
    const payload = await response.json() as { access_token?: string }
    if (!payload.access_token) throw new Error('Copernicus user authentication returned no access token.')
  ...
  const response = await fetch(TOKEN_URL, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body })
  if (!response.ok) throw new Error(`Copernicus authentication returned HTTP ${response.status}.`)
  const payload = await response.json() as { access_token?: string }
  if (!payload.access_token) throw new Error('Copernicus authentication returned no access token.')
  return payload.access_token
}

export async function fetchProcessImage(request: SearchRequest) {
  const token = await getCopernicusToken()
  if (!token) throw new Error('Copernicus Process API access requires CDSE_ACCESS_TOKEN or CDSE_USERNAME/CDSE_PASSWORD in server/.env.')
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (image.byteLength <= MAX_IMAGE_BYTES) break
    width = Math.max(1, Math.floor(width * 0.7))
    height = Math.max(1, Math.floor(height * 0.7))
  }

  if (!image || image.byteLength > MAX_IMAGE_BYTES) throw new Error('Copernicus Process API image could not be kept under 4 MB.')
...
function boundingBox(geometry: Feature<Polygon>) {
  const coordinates = geometry.geometry.coordinates[0]
  const longitudes = coordinates.map(([longitude]) => longitude)
  const latitudes = coordinates.map(([, latitude]) => latitude)
  return [Math.min(...longitudes), Math.min(...latitudes), Math.max(...longitudes), Math.max(...latitudes)] as const
}

function processDimensions(west: number, south: number, east: number, north: number) {
...
export function validateSearchRequest(value: unknown): SearchRequest {
  ...
  if (!request?.geometry || request.geometry.type !== 'Feature' || request.geometry.geometry?.type !== 'Polygon') throw new Error('A Polygon GeoJSON feature is required.')
  if (!isDate(request.startDate ?? '') || !isDate(request.endDate ?? '')) throw new Error('startDate and endDate must use YYYY-MM-DD format.')
  const startDate = request.startDate
  const endDate = request.endDate
  if (!startDate || !endDate) throw new Error('startDate and endDate are required.')
  if (startDate > endDate) throw new Error('startDate must be before endDate.')
  if (request.collection !== 'SENTINEL-1' && request.collection !== 'SENTINEL-2') throw new Error('collection must be SENTINEL-1 or SENTINEL-2.')
